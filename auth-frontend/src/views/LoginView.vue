  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <form @submit.prevent="handleLogin">
        ...
        <!-- RETO B: Checkbox para recordar el usuario -->
      ...
      <!-- RETO C: Link para recuperar contraseña -->
      <p class="forgot-link">
        ¿Olvidaste tu contraseña? 
        <router-link to="/forgot-password">Recuperar aquí</router-link>
      </p>
    </div>
  </div>
</template>
...
// RETO B: Variable para el checkbox de "Recuérdame"
// Si está marcado, guardamos el email en localStorage
const rememberMe = ref(false)

/**
 * RETO B: Cargar email guardado cuando se abre la página de login
 * Se ejecuta cuando el componente se monta (aparece en la pantalla)
 */
const loadSavedEmail = () => {
  // Buscar en localStorage si hay un email guardado
  // localStorage es como una carpeta donde guardamos cosas en la computadora
  const savedEmail = localStorage.getItem('remembered_email')
  
  // Si encontramos un email guardado, llenarlo automáticamente
  if (savedEmail) {
    email.value = savedEmail
    rememberMe.value = true  // Marcar el checkbox automáticamente
  }
}
...
const handleLogin = async () => {
  ...
  try {
    ...
    // RETO B: Si el usuario marcó "Recuérdame", guardar el email
    // Si no está marcado, eliminar el email guardado
    if (rememberMe.value) {
      // Guardar el email en localStorage (en la computadora)
      localStorage.setItem('remembered_email', email.value)
    } else {
      // Eliminar el email guardado si el checkbox no está marcado
...
// RETO B: Ejecutar la función para cargar el email guardado cuando la página se abre
// onMounted es un "gancho" que se ejecuta una sola vez cuando aparece el componente
onMounted(() => {
  loadSavedEmail()
})
</script>
...
/* RETO B: Estilos para el checkbox de recordarme */
.checkbox-group {
  display: flex;
  align-items: center;
  margin-bottom: 20px;
...
/* RETO C: Estilo para el link de recuperar contraseña */
.forgot-link {
  text-align: center;
  margin-top: 10px;
  color: #666;
